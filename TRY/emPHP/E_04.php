...
if($item['IP'] != $_SERVER["REMOTE_ADDR"]) echo "<p style='color:red;'>IP位置不正確，無法簽到</p>";
else{
	...
	if($H>3){
		echo "<p  style='color:red;'>現在並非您可以簽到的時間。</p>";
	}
	else if($H>=0){
		$sql="SELECT * FROM checklist WHERE id_number='$id_number' AND date='$today'";
		$result=$db_conn->query($sql);
		...
		foreach($user AS $item){
			if($item['morning']==0){
				...
				echo "<b  style='color:orange;'>已有簽到記錄!</b>";
			}
		}
		...
		if(count($user)==0){
			...
			foreach($user AS $item){
				if($item['morning']==0){
					...
					$result=$db_conn->query($sql);
					echo "<b  style='color:orange;'>加油，已補簽完畢!";
				}
				else{
					echo "<b  style='color:orange;'>已有簽到記錄!</b>";
				}
